...
class Talk extends Base {
  ...
  getTalkText(): string {
    ...
    if (this.body.msg_type != message.ChatMsgTypeText) {
      ...
      this.body.extra.content && (this.body.extra.content = Crypto.Msg.decrypt(this.body.extra.content))
  ...
  insertTalkRecord() {
    ...
    this.body.extra.content && (this.body.extra.content = Crypto.Msg.decrypt(this.body.extra.content))
    this.body.quote.content && (this.body.quote.content = Crypto.Msg.decrypt(this.body.quote.content))
    // 群成员变化的消息，需要更新群成员列表

...
export const useTalkRecord = (uid: number) => {
  ...
  const load = async (params: Params) => {
    ...
    for(let i=0;i< data.items.length;i++){
      let msg = data.items[i].extra.content
      if(msg){
        data.items[i].extra.content = Crypto.Msg.decrypt(msg)
      }
      let quoteMsg = data.items[i].quote.content
      if(quoteMsg){
        data.items[i].quote.content = Crypto.Msg.decrypt(quoteMsg)
      }
    }
